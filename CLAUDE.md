...
- **Oracle 단일 DB + ORM 경계** — 전 테이블은 `tools/models.py`(SQLAlchemy 2.x)에 선언, 생성은 `tools/db.py`의 `init_schema()`(서버 기동 시 create_all + Oracle Text 인덱스 + 시드, 멱등). **규약: 단순 CRUD(레지스트리·계정·설정·기록)는 ORM(`db.session()`), 복잡한 검색(CONTAINS·RRF)·MERGE 업서트·대량 배치·PL/SQL·체크포인터는 raw SQL 유지** — 18c/19c 대상, 23ai 전환 시 VECTOR 컬럼만 모델에 추가(SQLAlchemy 2.0.41+). 구버전 DB의 ALTER 마이그레이션은 각 모듈 ensure가 담당. raw 접속 상수는 `tools/blog_search.py`에서 import하는 게 관례. `lg_checkpoints`/`lg_writes`는 체크포인터 소유(모델 없음).
