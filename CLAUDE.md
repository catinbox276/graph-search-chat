...
규칙(불변식): **도메인 외부에서는 패키지 루트만 import**(`from core import config`, 깊은 경로 지양). **`common.py`/`utils.py`/번호 접미사 금지** — 커지면 유스케이스 단위로 쪼갠다(줄 수 아님, 변경 이유 2개 이상이면 분리). 파일이 커지면 `search/corpus_search.py` → `search/corpus_search/` 패키지로 승격하고 `__init__`에서 re-export(외부 import 경로 불변). **승격한 패키지 안은 역할명 파일로 나눈다 — 한 파일 = 한 역할**(모델 호출·SQL·룰 판정이 한 파일에 섞이지 않게. 예: `graph/graph_pipeline/`의 schema(DDL·시드)·llm(모델 호출)·gate(룰 판정)·merge(dedup)·weights(보정)·run(오케스트레이션), 배치 엔트리는 `__main__.py`). `core/`는 모든 도메인이 의존하는 공용만 — 도메인 로직 금지.
...
- **에이전트** — `agent/agent.py`가 DeepAgents로 조립. 툴 = `suggest_paths`(새 문제 시 최우선 호출, 시스템 프롬프트로 강제) + `search_docs`/`read_doc`(함수 직접 등록 — 통합 코퍼스 검색·열람, 구명 search_blog/read_blog_post에서 개명) + **소스별 검색 도구 자동 생성**(`search_{소스명}` — source_registry 등록 = 검색 툴 등록) + **mcp_registry에 등록된 도구 서버들**(streamable_http/sse/stdio + **rest** — 사내 `GET /tools`+`POST /call` 어댑터 `core/rest_tools.py`. GMS 직결(stdio)은 사내에서 응답이 느려 실사용 불가 실측(2026-08-13) → rest 방식 채택. 시드 없음 — 관리 페이지나 MCP_DEFAULT_URL로 등록, 주소 등록만으로 도구 자동 조립). 모델별 에이전트 캐시는 server.py의 `_agents`. **전역 제어는 관리 페이지 /admin > 에이전트 설정**(app_settings: `agent_system_prompt` 덮어쓰기·`agent_mcp_enabled`·`agent_disabled_tools` — 저장 시 캐시 무효화로 다음 질문부터 반영, MCP 서버 주소는 .env 소관).
...
## 문서
- `docs/plan.md` — 기획서 v3 (2026-08-12: 코드 기반 팩트만 — 데이터 2종 처리·하이브리드 검색·구조화·클러스터·대화 자산화·DataHub REST 도구. 공유용, 쉬운 설명). 이전 판본은 `docs/plan-v1.md`(초기 기획·연구 근거)·`docs/plan-v2.md`(PoC 후 개정) 아카이브
