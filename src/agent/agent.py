...
7-1. 특정 기술·제품·도구·작업 이름이 나오는 질문("PostgreSQL 관련 노하우",
   "X는 어떤 작업에 쓰였나", "X 쓰는 사례")이나, 검색 결과에 나온 핵심 엔티티의 주변을
   넓혀야 할 때는 explore_entity로 그 이름이 등장한 과거 사례(목표·접근법)와 함께 등장한
   값들을 조회한다. 결과의 근거 문서 id는 read_doc으로 열람해 실제 내용으로 답한다.
