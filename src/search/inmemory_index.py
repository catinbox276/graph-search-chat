...
_node_meta = {}        # node_cid(int) -> (node_id, layer) — 그래프 노드 하이브리드용
_has_gvec = False      # 노드 벡터 테이블 존재 여부
...
def _current_version(cur) -> tuple:
    ...
    cur.execute("SELECT COUNT(*), MAX(name) FROM nodes")  # 노드 변경 감지 (전 계층)
    goals = tuple(cur.fetchone())
    return chunks + goals


def _load_nodes(cur):
    """(node_id, layer, name, unit_vec) — 전 계층 그래프 노드. 하이브리드 인덱스용
    (2층은 경로 제안의 그래프 진입, 전 계층은 그래프 뷰 검색).
    nodes.embedding은 JSON 인코딩(청크의 raw float32와 다름) → json.loads.
    벡터는 단위벡터로 정규화 — sqlite-vec 기본 L2 거리를 코사인으로 환산하기 위함."""
    import json
    cur.execute("SELECT id, layer, name, embedding FROM nodes WHERE name IS NOT NULL")
    for nid, layer, name, blob in cur.fetchall():
...
def build_index():
    ...
    global _con, _meta, _version, _has_vec, _node_meta, _has_gvec
    ...
    with _lock:
        ...
        # --- 그래프 노드(전 계층) 인덱스 — 문서 검색과 같은 FTS5+sqlite-vec 재사용 ---
        ...
        for gcid, (nid, layer, gname, gvec) in enumerate(gnodes):
            ...
            gmeta[gcid] = (nid, layer)

        con.commit()
        _con, _meta, _version, _has_vec = con, meta, version, dim is not None
        _node_meta, _has_gvec = gmeta, gvec_on
        print(f"[inmemory_index] 빌드: {len(meta)}청크 + {len(gmeta)}노드 "
              f"(벡터 dim={dim}, 노드벡터={gvec_on}, ver={version})")
    return len(_meta)
...
def node_lexical(query: str, n: int, layers=None) -> list:
    """그래프 노드 렉시컬(FTS5 BM25) → [node_id...].
    layers: 계층 필터 집합 (예: {2} = 목표만 — 경로 제안의 그래프 진입). None이면 전 계층."""
    if _con is None:
        return []
    q = _fts_query(query)
    if not q:
    ...
    rows = _con.execute("SELECT cid FROM gfts WHERE gfts MATCH ? ORDER BY bm25(gfts) LIMIT ?",
                        (q, n * 5 if layers else n)).fetchall()  # 필터 감안 여유분
    out = []
    for (cid,) in rows:
        nid, layer = _node_meta[cid]
...
def node_semantic(query: str, n: int, min_cos: float, layers=None) -> list:
    """그래프 노드 시맨틱(sqlite-vec 코사인) → [node_id...], 코사인 ≥ min_cos만.
    layers: 계층 필터 집합 (None이면 전 계층).
    ...
    try:
        rows = _con.execute(
            ...
            (qv, n * 5 if layers else n)).fetchall()
...
if __name__ == "__main__":
    ...
    print("goals:", node_lexical("환불 규정", 5, layers={2}))
