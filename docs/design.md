...
flowchart TD
    ...
    Q["신규 사용자 질의"] --> CL["2층(목표) 노드 하이브리드 매칭<br/>(SQLite 인메모리 FTS5 + sqlite-vec, RRF)"]
    CL --> TR["진입 목표에서 그래프 순회 (≤3 hop)"]
    TR --> G
    G -.->|"에피소드 ID 조인"| EV
...
### 별도 벡터 검색 서버를 불필요하게 만드는 두 가지 장치
1. **진입점 = 인메모리 하이브리드 매칭** (2026-08-12 갱신 — 초기 설계는 LLM 분류였으나, 문서 검색용 SQLite `:memory:` 인덱스가 이미 있어 2층 목표 노드를 같은 인덱스(FTS5 BM25 + sqlite-vec)에 얹는 것이 LLM 호출 1회보다 싸고 빠르며 정확 용어(렉시컬)와 유사 의미(시맨틱)를 모두 잡는다). 렉시컬+시맨틱을 RRF로 융합해 진입 목표를 고르고, 거기서부터 그래프 순회. 임베딩 미서빙 시 렉시컬 단독 폴백으로 진입이 끊기지 않는다.
