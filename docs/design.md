...
5. **낡은 건 지우지 말고 가라앉힌다 (시간 감쇠)** — 오래 통행이 없는 접근법은 가중치를 감쇠시켜 추천 순위에서 내리되 삭제하지 않는다. 신선도는 원인(문제)이 아니라 **조치(접근법)**에 붙는다 — 문제는 안 변하고 해법이 낡는다. 직접 증거: AWM 표11 — 분포가 다른 오래된 워크플로우를 새 것과 합치자 성공률이 4.8→1.6으로 급락 (아카이브 "AWM 논문 정독"). 유지보수 배치에 감쇠 패스 추가가 다음 구현 과제.
...
| 미분류 유지 | dedup 애매 구간(0.70~0.92)은 LLM 동일 의도 확인, 그래도 애매하면 병합하지 않고 남긴다 — 억지로 가르지 않음 |
