...
- 연결 방식은 **REST로 감싼 MCP**다: `GET {주소}/tools`(도구 목록) + `POST {주소}/call`
  (실행). GMS에 공식 MCP를 직결하는 방식도 검증했으나 **사내 GMS 응답이 느려 실사용
  불가**로 판명(2026-08-13 실측) — REST 어댑터가 도구 정의를 손실 없이 1:1 변환한다.
  관리 콘솔에서 주소만 등록하면 도구가 자동으로 에이전트에 조립된다.
- 도구 서버가 하나 죽어도 나머지 도구는 동작한다(서버별 격리). 도구 실행 오류는 대화를
  죽이지 않고 에이전트가 우회를 판단한다.
- 사내 실서버 연동은 REST 방식으로 확인 — 도구 목록·호출 형식이 공식 MCP와 1:1 대응한다.
...
### "병합한다"는 게 정확히 뭘 하는 건가

텍스트를 합치거나 이름을 새로 짓는 게 아니다. 하는 일은 딱 세 가지:

1. **새 노드를 만들지 않는다** — 새 문구는 저장되지 않고, 기존 노드("프록시 환경 pip 설치
   실패")가 그 지식을 대표한다. **이름은 먼저 생긴 노드 것이 그대로 유지**된다.
2. **연결 횟수 +1** — 그 노드로 가는 연결선의 통행 수(가중치)가 올라간다.
3. **출처 기록** — 이 문구가 어느 세션/문서에서 왔는지 증거 테이블에 남는다.
   나중에 "이 병합이 맞았나"를 원본으로 되짚을 수 있다.

### 왜 0.92 이상이면 LLM한테 안 물어보나

먼저: **0.92는 "92% 정확도"가 아니다.** 두 문장이 의미 공간에서 얼마나 가까운지를 나타내는
거리 값(코사인 유사도)이고, 확률과는 다른 척도다. LLM을 생략하는 근거는 세 겹이다:

1. **실측해 보니 그 구간엔 오답이 없었다** — 지금 쓰는 임베딩 모델로 재 보면, *같은 의도*를
   다르게 표현한 쌍은 0.81~0.83, *다른 의도*의 쌍은 0.34~0.46이 나온다. 두 분포 사이가
   확 벌어져 있고, 0.92는 "같은 의도" 쪽보다도 한참 위다 — 이 높이까지 올라왔는데 알고 보니
   다른 의도였던 쌍은 실측에서 나오지 않았다. (그래서 이 숫자는 모델의 함수다 —
   임베딩 모델을 바꾸면 반드시 다시 잰다.)
2. **문자열 검증이 한 겹 더 있다** — 0.92를 넘어도 "둘 다 12자 이상 + 문자 유사도 통과"를
   추가로 요구한다. 짧은 문구에서 임베딩이 과신하는 경우를 여기서 걸러낸다. 즉 "0.92면
   무조건 병합"이 아니라 **두 검사를 다 통과한 것만** 자동 병합이고, 하나라도 걸리면
   LLM 판정으로 내려보낸다.
3. **확실한 구간에 LLM을 끼우면 얻는 것보다 잃는 게 많다** — 구조화는 수천~수만 건
   배치라 전 건 LLM 확인이면 처리량이 반토막 나고, LLM 판정 자체도 흔들린다(같은 질문에
   다른 답을 줄 수 있다). 실측상 거의 확실한 구간에서는 LLM이 "확인"이 아니라 노이즈가
   된다. "확실한 건 기계적으로, 애매한 것만 LLM에게"는 업계가 수렴한 구조이기도 하다
   (LLM 단독 병합으로 시작한 오픈소스 Graphiti도 이 구조로 후퇴했다).

그래도 잘못 병합되면: 출처가 남아 있어 추적 가능하고, **야간 유지보수 배치**가 2차로
정리하며(저빈도 형제 통합·오래된 잎 흡수), 임계값은 설정으로 조절할 수 있다.

그 밖의 정확한 동작 (코드 확인 사항):
...
flowchart TB
    subgraph day["낮 (실시간)"]
        ...
        AG -->|"데이터 질문"| DH["DataHub 도구<br/>(공식 MCP — GMS 직결)"]
