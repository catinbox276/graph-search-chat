# 기획 보고 v1 (아카이브) — 사내 에이전트와 "대화 자산화" 프로젝트

> **작성 2026-08-05 · v1 — 보존용 아카이브.** 최신 기획은 [plan.md](plan.md)(v3), 중간 개정판은 [plan-v2.md](plan-v2.md).
...
우리는 DataHub의 MCP(도구 연결 표준)로 에이전트가 "이런 데이터 어디 있어?"를
탐색하게 한다. (참고: https://datahubproject.io , MCP: https://github.com/acryldata/mcp-server-datahub )

- **연결 방식**: MCP 서버는 **사내에 이미 배포되어 제공 중**이라 우리가 만들 게 없다 —
  HTTP로 붙기만 하면 된다. `GET http://<서버>/tools`로 도구 목록을 받아오고,
  `POST http://<서버>/call`로 도구에 파라미터를 넣어 실행·응답을 받는다.
- **사용 도구**: 서버가 제공하는 총 8개 중 **읽기 전용 5개만** 사용한다.

  | 도구 | 하는 일 |
  |---|---|
  | `search` | 데이터셋(테이블) 검색 |
  | `get_entities` | 테이블 상세·스키마 설명 조회 |
  | `list_schema_fields` | 컬럼(필드) 목록 조회 |
  | `get_lineage` | 데이터가 어디서 흘러와 어디로 가는지(리니지) 추적 |
  | `get_dataset_queries` | 그 테이블에 실제로 쓰였던 쿼리 예시 조회 |

  쓰기(메타데이터 수정) 계열 도구는 일부러 쓰지 않는다 — 에이전트가 카탈로그를
  오염시킬 경로를 처음부터 차단하기 위해서다.
...
> 구현 현황: 검색 코퍼스화(역할 매핑 조립)와 **LLM 그래프 구조화(도메인 기준 판정→추출→병합,
> 미달 제외)** 모두 동작한다 — 소스 관리 UI에서 "그래프 도메인"을 지정하면 야간 배치가 처리한다.
...
| 4 | DB 간 도메인 관계 정의·탐색 (DataHub 한계 보완) | 추후 |
| 5 | 축적 로그의 2차 활용 검토 (도메인 특화 학습 등 — §4-3 한계 고려하에) | 추후 |
