...
`kind`(llm/embedding/reranker) + `name` 복합 PK, `enabled`, `is_default`, `base_url`(호스트별 서빙 분리 — 다중 호스트 동기화 지원). 임베딩 기본 모델이 여기서 결정 — 청크 임베딩의 `embed_model` 값과 연결 (아래 §5). 서빙 동기화는 능력 테스트(임베딩→리랭크→챗 순 실호출, 응답 본문 기준 판정)로 kind를 정하고 종류별 기본을 동작 검증된 모델로 자동 지정한다.
...
그래프는 구조적으로 성장에 강함 — dedup은 전체가 아니라 **부모당 형제 수**에 비례. 경로 진입점(2층 노드를 SQLite 인메모리 FTS5+sqlite-vec 하이브리드로 매칭)만 2층 수만 개 시점에 위와 같은 처방(사전 필터 → 23ai). 배치는 증분 워터마크·멱등 설계라 동시성 설정 조절이 전부.
